...
function imageToAscii(img, width = 120, frameOffset = 0) {
    const chars = 'ViệtNam'; // Sử dụng cụm từ "Việt Nam "
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const aspectRatio = img.height / img.width;
    canvas.width = width;
    // Tính chiều cao canvas dựa trên tỷ lệ khung hình gốc của ảnh
    ...
    for (let y = 0; y < canvas.height; y++) {
        ...
        for (let x = 0; x < canvas.width; x++) {
            ...
            // Lấy ký tự tiếp theo từ chuỗi 'Việt Nam ', có tính đến độ lệch khung hình
            const char = chars[(charCounter + frameOffset) % chars.length];
            row.push({
                char: char,
                color: `rgb(${r},${g},${b})`
            });
            charCounter++; // Tăng bộ đếm
        }
        ascii.push(row);
    }
    return ascii;
    ...
    if (file.type.startsWith('image/')) {
        ...
        video.addEventListener('play', function () {
            ...
            function step() {
                ...
                let charHeightApprox = 8; // Mặc định
                ...
                // Chiều cao tương ứng dựa trên tỷ lệ khung hình video và tỷ lệ ký tự
                let asciiHeight = Math.round(asciiWidth / videoAspectRatio);
